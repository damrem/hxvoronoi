...
class Voronoi {
	...
	private var _sitesIndexedByLocation:Map<String, Site>;
	...
	{	
		...
		_sites = new SiteList();
		_sitesIndexedByLocation = new Map<String, Site>();
...
//		}
	...
	private function addSite(p:Point, color:UInt, index:Int):Void
	{
		...
		_sites.push(site);
		_sitesIndexedByLocation.set(p.toString(), site);
	}
	...
	{
		var site:Site = _sitesIndexedByLocation.get(p.toString());
		if (site == null)
		{
	...
	public function neighborSitesForSite(coord:Point):Array<Point>
	{
		...
		var site:Site = _sitesIndexedByLocation.get(coord.toString());
		if (site == null)
		{
			return points;
		}
